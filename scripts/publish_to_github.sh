...
if [[ -z "${REMOTE_URL}" ]]; then
  ...
  echo "Example: $0 https://github.com/Milad-Shabani/Covid-vaccination-demand-forecasting.git" >&2
  exit 1
fi
...
if git diff --cached --quiet; then
  ...
  echo "==> Committing..."
  git commit -m "Malard VaxForecast: synthetic vaccination operations dataset + SARIMA/XGBoost forecasting"
fi
...
if ! git push -u origin "${BRANCH}"; then
  echo "==> Push rejected (remote has commits this local repo doesn't). Pulling and rebasing..."
